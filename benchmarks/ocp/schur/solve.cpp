...
void solve(SchurFactor &factor) {
    ...
    linalg::xtrsv<real_t, index_t>(CblasColMajor, CblasLower, CblasNoTrans,
                                   CblasNonUnit, vN.rows(), LN.data(),
                                   LN.outerStride(), vN.data(), index_t{1});
    // e + Wv
    auto WᵀN = factor.Wᵀ(ocp.N).topRows(ocp.nx);
    λN       = ocp.es.col(ocp.N);
    linalg::xgemv<real_t, index_t>(CblasColMajor, CblasTrans, WᵀN.rows(),
                                   WᵀN.cols(), real_t{1}, WᵀN.data(),
                                   WᵀN.outerStride(), vN.data(), index_t{1},
                                   real_t{1}, λN.data(), index_t{1});
    for (index_t k = ocp.N; k-- > 0;) {
        ...
        // L v = g
        linalg::xtrsv<real_t, index_t>(CblasColMajor, CblasLower, CblasNoTrans,
                                       CblasNonUnit, vj.rows(), Lj.data(),
                                       Lj.outerStride(), vj.data(), index_t{1});
        // e + Wv
        auto Wᵀj = factor.Wᵀ(k);
        λj       = ocp.es.col(k);
        linalg::xgemv<real_t, index_t>(CblasColMajor, CblasTrans, Wᵀj.rows(),
                                       Wᵀj.cols(), real_t{1}, Wᵀj.data(),
                                       Wᵀj.outerStride(), vj.data(), index_t{1},
                                       real_t{1}, λj.data(), index_t{1});
        // e(j+1) + Wv(j+1) - Vv(j)
        linalg::xgemv<real_t, index_t>(CblasColMajor, CblasNoTrans, Vj.rows(),
                                       Vj.cols(), real_t{-1}, Vj.data(),
                                       Vj.outerStride(), vj.data(), index_t{1},
                                       real_t{1}, λ_next.data(), index_t{1});
        if (k + 1 < ocp.N) {
    ...
    for (index_t k = 0; k < ocp.N; ++k) {
        ...
        auto vj      = factor.v.col(k);
        linalg::xgemv<real_t, index_t>(CblasColMajor, CblasNoTrans, Wᵀj.rows(),
                                       Wᵀj.cols(), real_t{1}, Wᵀj.data(),
                                       Wᵀj.outerStride(), λj.data(), index_t{1},
                                       real_t{-1}, vj.data(), index_t{1});
        linalg::xgemv<real_t, index_t>(
